<div class="block full">
	...
	<div class="block-title">
		...
		<h2><i class="hi hi-cog"></i> <strong>DATA MASTER MARKETING</strong>
		</h2>
	</div>
				...
				<table id="example-datatable" class="table table-vcenter table-condensed table-bordered table-hover">
					<thead>
						<tr>
							...
							<th class="text-center" style="width: 10%;">NOMOR ID</th>
					...
					<tbody>
						<?php
						$i = 1;
						$wk = $this->db->order_by('nama', 'ASC')->get_where('wilayah_kerja', array('dihapus' => 'tidak'))->result();
						$j = $this->db->order_by('id', 'ASC')->get_where('jabatan', array('dihapus' => 'tidak'))->result();
						...
						?>
							<tr>
								...
								<?php
									...
									echo (isset($w[0]->nama) ? ($w[0]->nama.' '.strtoupper($w[0]->provinsi)) : ''); 							
								?>
								</td>
								<td><?= $data->tlp; ?></td>
								<td><?= $data->email; ?></td>
								<td class="text-center">
									<div class="btn-group btn-group-xs">
										<a href="<?= site_url('admin/master/marketing/detail/' . $data->id . '/detail-mitra') ?>" data-toggle="tooltip" title="Detail" class="btn btn-xs btn-primary">
											<i class="fa fa-eye"></i>
										</a>
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-header text-center">
				<h2 class="modal-title"><i class="fa fa-child"></i> Tambah Marketing Baru</h2>
			</div>
			<!-- END Modal Header -->
			<!-- Modal Body -->
			<div class="modal-body">
				...
				<fieldset>
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="nomor_id">Nomor ID <span class="text-danger">*</span></label>
						<div class="col-md-6">
							<input type="text" id="nomor_id" name="nomor_id" class="form-control" placeholder=".." required>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="nama_lengkap">Nama Lengkap <span class="text-danger">*</span></label>
						<div class="col-md-6">
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="jabatan">Jabatan <span class="text-danger">*</span></label>
						<div class="col-md-6">
							<select id="jabatan" name="jabatan" class="form-control select-chosen" style="width:100%;" required>
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="wilayah">Wilayah Kerja <span class="text-danger">*</span></label>
						<div class="col-md-6">
							<select id="wilayah" name="wilayah" class="form-control select-chosen" style="width:100%;" required>
								...
								?>
									<option value="<?= $wks->id; ?>">
										<?= $wks->kode.' '.$wks->nama.' ('.strtoupper($wks->provinsi).')'; ?>
									</option>
								<?php
								}
								?>								
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="tanggal_mulai_bermitra">Tanggal Mulai Bermitra </label>
						<div class="col-md-4">
							<div class="input-group">
								<span class="input-group-addon text-info"><i class="fa fa-calendar"></i></span>
								<div class="input-date" data-date-format="yyy-mm-dd">
									<input type="text" id="tanggal_mulai_bermitra" name="tanggal_mulai_bermitra" class="form-control input-datepicker" data-date-format="yyyy-mm-dd" placeholder="Tanggal Mulai Bermitra" autocomplete="off">
								</div>
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						<div class="col-md-6">
							<label class="col-md-4 control-label" for="tlp">Telepon </label>
							<div class="col-md-8">
								<input type="text" id="tlp" name="tlp" class="form-control" placeholder="..">
							</div>
						</div>
						<div class="col-md-6">
...
<div class="modal fade" id="FormEdit" tabindex="-1" role="dialog" aria-labelledby="modal-fadein" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<!-- Modal Header -->
			<div class="modal-header text-center">
				<h2 class="modal-title"><i class="fa fa-pencil"></i> Edit Data Marketing</h2>
			</div>
			<!-- END Modal Header -->
			<!-- Modal Body -->
			<div class="modal-body">
				<?php 
				echo form_open($action_edit, array('class' => 'form-horizontal form-bordered')); 
				...
				<fieldset>
					...
					<div class="form-group">
						...
						<div class="form-group">
							<label class="col-md-3 control-label" for="jabatan">Jabatan <span class="text-danger">*</span></label>
							<div class="col-md-6">
								<select id="jabatan" name="jabatan" class="jabatan form-control select-chosen" style="width:100%;" required>
									<option value="" selected disabled>-- Pilih Jabatan--</option>								
									<?php
									//j = jabatan
									foreach ($j as $js) {
						...
						<div class="form-group">
							<label class="col-md-3 control-label" for="wilayah">Wilayah Kerja <span class="text-danger">*</span></label>
							<div class="col-md-6">
								<select id="wilayah" name="wilayah" class="wilayah form-control select-chosen" style="width:100%;">
									<option value="" selected disabled>-- Pilih Wilayah Kerja --</option>								
									<?php
									//wk = wilayah kerja
									foreach ($wk as $wks) {
						...
						<div class="form-group">
							<div class="col-md-6">
								<label class="col-md-4 control-label" for="tlp">Telepon <span class="text-danger">*</span></label>
								<div class="col-md-8">
									<div class="input-group">
...
<script>
	...
	function detail(id) {
        ...
        $.ajax({
            url: "<?php echo site_url('admin/master/marketing/ajax_get_detail') ?>",
            type: "POST",
            data: 'id=' + id,
            dataType: "json",
            success: function(data) {
                $('[name="id"]').val(id);
                $('[name="nomor_id"]').val(data.nomor_id);
                $('[name="nama_lengkap"]').val(data.nama_lengkap);
                //$('[name="wilayah"]').val(data.wilayah_kerja);
                $('[name="tlp"]').val(data.tlp);
                $('[name="alamat"]').val(data.alamat);
                $('[name="tanggal_mulai_bermitra"]').val(data.tanggal_mulai_bermitra);
				...
				$(".wilayah").val(data.wilayah_kerja).trigger("chosen:updated");
				$(".jabatan").val(data.jabatan).trigger("chosen:updated");
	...
	$(document).on('blur', '#nomor_id', function(e) {
		...
		$.ajax({
			url: "<?php echo site_url('admin/master/marketing/cek_kode') ?>",
			type: "POST",
			cache: false,
